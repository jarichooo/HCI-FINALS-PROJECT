...
function ChangePasswordModal({ onClose }) {
  ...
  const set = (k, v) => { setFields((f) => ({ ...f, [k]: v })); setStatus(""); };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!fields.current || !fields.next || !fields.confirm) { setErrMsg("Please fill in all fields."); setStatus("error"); return; }
    if (fields.next.length < 8) { setErrMsg("New password must be at least 8 characters."); setStatus("error"); return; }
    if (fields.next !== fields.confirm) { setErrMsg("New passwords do not match."); setStatus("error"); return; }
    setStatus("success");
  };

  const PwField = ({ label, fk }) => (
    <div style={{ marginBottom: "12px" }}>
      <label style={f.label}>{label}</label>
      <div style={f.row}>
        <input type={show[fk] ? "text" : "password"} value={fields[fk]} onChange={(e) => set(fk, e.target.value)} style={f.input} placeholder="••••••••" />
        <button type="button" style={f.eye} onClick={() => setShow((s) => ({ ...s, [fk]: !s[fk] }))}><EyeIcon open={show[fk]} /></button>
      </div>
    </div>
  );
    ...
    <Modal title="Change Password"
      ...
      {status === "success"
        ...
        : <form onSubmit={handleSubmit}>
            <PwField label="Current Password" fk="current" />
            <PwField label="New Password" fk="next" />
            <PwField label="Confirm New Password" fk="confirm" />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "9px" }}>
          {features.map(([emoji, label, desc]) => (
            <div key={label} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
              <span style={{ fontSize: "17px", flexShrink: 0 }}>{emoji}</span>
              <div>
                <p style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-primary)", transition: "color 0.25s" }}>{label}</p>
                <p style={{ fontSize: "12px", color: "var(--text-secondary)", marginTop: "1px", transition: "color 0.25s" }}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
...
// ─── Main Export: Settings gear button + dropdown ─────────────────────────────
        ...
        {open && (
          <div style={s.dropdown}>
            ...
            {[
              ...
              <button key={item.key} style={{ ...s.dropItem, borderBottom: i < arr.length - 1 ? "1px solid var(--border-light)" : "none" }}
